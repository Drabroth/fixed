-- AutoLootBody combined compatibility build
-- Base: user compatibility version + useful changes from maker update
-- Loot categories:
...
local function LootGm82_009(gimmick)
    ...
    local ok, result = pcall(function()
        ...
        -- Maker update: already-given gather spots must not be triggered again.
        local isGivedItem = false
        pcall(function()
            isGivedItem = gimmick.IsGivedItem == true
        end)
        if isGivedItem then
            return false
        end
        ...
        Log("GATHER",
            ...
            "IsGivedItem", tostring(isGivedItem)
        )
        ...
        -- Maker update uses Execute + End. Keep both, but protect them with pcall.
        local executeWorked, executeErr = pcall(function()
            gimmick:onExecuteInteractBase(0, player)
        end)
        Log("GATHER EXECUTE", executeWorked, executeErr)

        local endWorked, endErr = pcall(function()
            gimmick:onEndInteractBase(0, player)
        end)
        Log("GATHER END", endWorked, endErr)

        -- Compatibility fallback for game/REFramework versions where the above changed.
        if not executeWorked and not endWorked then
            local forceWorked, forceErr = pcall(function()
                gimmick:requestForceInteract(0, player)
            end)
            Log("GATHER FORCE", forceWorked, forceErr)
...
local function LootGm82_000_001(gimmick)
    ...
    local ok, result = pcall(function()
        ...
        -- Maker update: dropped items use Start -> Execute -> End.
        -- Keep that exact sequence, but protect every call so one changed method
        -- cannot kill the whole AutoLoot script.
        local startWorked, startErr = pcall(function()
            gimmick:onStartInteractBase(0, player)
        end)
        Log("DROP START", startWorked, startErr)

        local executeWorked, executeErr = pcall(function()
            gimmick:onExecuteInteractBase(0, player)
        end)
        Log("DROP EXECUTE", executeWorked, executeErr)

        local endWorked, endErr = pcall(function()
            gimmick:onEndInteractBase(0, player)
        end)
        Log("DROP END", endWorked, endErr)

        -- Extra compatibility fallback from the user's fixed version.
        if not executeWorked and not endWorked then
            local forceWorked, forceErr = pcall(function()
                gimmick:requestForceInteract(0, player)
            end)
            Log("DROP FORCE", forceWorked, forceErr)
            if not forceWorked and not startWorked then
                return false
            end
        end
...
-- Maker update moved back to GimmickManager:update(), but this combined build
-- deliberately keeps the already-tested InteractManager:onUpdate() compatibility path.
